...
@implementation JZYModel

#pragma mark - override method
+ (void)initialize
{
    if (self != [JZYModel self]) {
        [self createTable];
    }
}
...
{
    __block BOOL res = NO;
    JZYHelper *jkDB = [JZYHelper shareInstance];
...
+ (BOOL)createTable
{
    FMDatabase *db = [FMDatabase databaseWithPath:[JZYHelper dbPath]];
    if (![db open]) {
        NSLog(@"数据库打开失败!");
        return NO;
    }
...
{
    //判断是否是JKBaseModel的子类
    for (JZYModel *model in array) {
        if (![model isKindOfClass:[JZYModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            JZYModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
